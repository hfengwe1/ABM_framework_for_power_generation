...
for s in range(S):
    ...
    for t in Cost.index:
        # print(t)
        if t > 2021:
            Cost["Wind"][t] = Cost["Wind"][t-1] * (np.random.normal(rate,0.01))   # Wind cost
            Cost["Solar"][t] = Cost["Solar"][t-1]  * (np.random.normal(rate,0.03))  # Solar cost 
            Cost["NG"][t] = Cost["NG"][t-1] *  (np.random.normal(1,0.01))  # NG cost 
        else:  # t = 2021
            Cost["Wind"][t] = G_hist_cost["Wind"][20] * (np.random.normal(rate,0.01))  # Wind cost
            Cost["Solar"][t] = G_hist_cost["Solar"][20] * (np.random.normal(rate,0.03)) # Solar cost 
...
# read data for capacity deficit calculation
    ...
    for t in range(T): # only zonal demand data only available from 2021 to 2030
        agt_rec = (REC_p/2*(1-(1+t))/T)*rec_dist   # decreasing financial incentive
