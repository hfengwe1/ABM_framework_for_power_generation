...
from ABM_function import KGE_stat
...
# Month abbreviation, day and year	
date_now = today.strftime("%b-%d")  # date string
Result_dir = os.path.join(cwd,"00 Results","Calibration", "Calibration_IRR8_Optimal" + date_now ) # save results
...
IRR_threshold = 0.08  # investment threshold (IRR >= 8%)
...
cap_hist = CP.iloc[12:,:]  # historical new and total capacity - get data from 2012-2020
...
random_cost_dict = pd.read_csv(os.path.join(Parameter_dir,"Cost_dist_Sep-24_2.csv"))
agt_size_risk_dict = pd.read_csv(os.path.join(Parameter_dir,"agt_size_risk_dict_Sep-24_2.csv"))

for k in range(30):  # repeat 30 sumulations to estimate mean KGE
    ...
    for t in range(9): # only zonal demand data only available from 2011 to 2020
        ...
        for agt_i in range(n_agt):
            new_D = Demand.loc[y].values*np.random.normal(1, d_err) # Agent's new demand forecast
            new_cost = df_cost.iloc[:,1:][df_cost["Year($MW)"] == y]          # new cost estimates 
            # linear function Totoal capacity = total demand*0.00029 + 5638 (using data from 2010 to 2020
            ...
            for i in range(len(LZ)):
                ...
                IRR_t.append(max_IRR)  # append new at Zone LZ and time t to the list
    ...
    fuel_yr_file = os.path.join(Result_dir, "ABM_fuel_yr_"+str(k)+".xlsx")
