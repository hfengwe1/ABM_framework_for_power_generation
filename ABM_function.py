...
def generating_ABM_samples(new_dir, renewable, n_agt, date_now, k):
    # create a directory if not exist
    ...
    wind_cost_dist = list(np.random.normal(0.9,0.15,renewable)) + list(np.ones([1,n_agt-renewable])[0])  # the cost adjustment for renewable companies
    solar_cost_dist = list(np.random.normal(0.4,0.12,renewable)) + list(np.ones([1,n_agt-renewable])[0]) # the cost adjustment for renewable companies
